...
function sortByMonth(arr: MonthlyDataItem[]) {
...
export function faturamentoPorPlataforma(vendas: Venda[]): ChartDataItem[] {
  ...
  return Object.entries(map).map(([name, value]) => ({ name, value, percent: total > 0 ? (value / total) * 100 : 0 })).sort((a, b) => b.value - a.value)
}
...
export function faturamentoMensalPorChave(vendas: Venda[], chave: keyof Venda): MonthlyDataItem[] {
  ...
  return sortByMonth(Object.entries(map).map(([mes, vals]) => ({ mes, ...vals })))
}
...
export function cancelamentoPorGrupo(vendas: Venda[], chave: keyof Venda): ChartDataItem[] {
  ...
  return Object.entries(map)
    .map(([name, { total, cancelados }]) => ({ name, value: total > 0 ? (cancelados / total) * 100 : 0, percent: cancelados }))
...
export function cancelamentoMensalPorChave(vendas: Venda[], chave: keyof Venda): MonthlyDataItem[] {
  ...
  return sortByMonth(Object.entries(map).map(([mes, vals]) => {
...
export function calcAfiliados(vendas: Venda[]): AfiliadoData[] {
  ...
  return Object.entries(map)
    .map(([afiliado, { vendas, faturamento }]) => ({ afiliado, vendas, faturamento, percentual: totalFat > 0 ? (faturamento / totalFat) * 100 : 0 }))
...
export const PRODUTO_COLORS = ['#0F482F', '#779E39', '#c8874a', '#5a7a2a', '#b05a20']
export const MUTED_COLOR = '#D6CDB8'
